...
class EcgData():
    ...
    def autocorrelation(self, file_number):
        ...
        acorr_peaks_index = find_peaks_cwt(autocorr_window, np.arange(30, 700), min_length=samples/max_time*.07)
        # peaks_index = find_peaks_cwt(voltages, np.arange(int(sample_range['min_spb']), int(sample_range['max_spb'])),
        #                              min_length=samples / max_time * .08, noise_perc=5)
        peaks_index = find_peaks_cwt(voltages, np.arange(9, 700),
                                     min_length=samples / max_time * .08, noise_perc=5)
